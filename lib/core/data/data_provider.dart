...
class DataProvider extends ChangeNotifier {
  ...
  void filteredVariants(String keyword) {
    if (keyword.isEmpty) {
      _filteredVariants = List.from(_allVariants);
    } else {
      final lowerKeyword = keyword.toLowerCase();
  ...
  void filterOrders(String keyword) {
    if (keyword.isEmpty) {
      ...
      _filteredOrders = _allOrders.where((order) {
        ...
        return newMatches || statusMatches;
      }).toList();
    }
    notifyListeners();
  }

  Future<List<MyNotification>> getAllNotification(
  ...
  int calulateOrderWithStatus({String? status}) {
    int totalOrders = 0;
    if (status == null) {
  ...
  void filterProducByQuantity(String productQntType) {
  ...
  int calculateProductWithQuantity({int? quantity}) {
    int totalProduct = 0;
    //if quantity is null, return total product
